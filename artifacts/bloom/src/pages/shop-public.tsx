...
type Product = { id: string; title: string; description?: string; slug: string; price_cents: number; promo_price_cents?: number | null; currency: string; category?: string };
...
export default function ShopPublicPage() {
  ...
  return <div className="content-wrap public-shop"><Link href="/discover" className="button button-ghost"><ArrowLeft size={15} /> Découvrir</Link><div className="public-shop-head"><div className="shop-avatar"><Store size={27} /></div><p className="kicker">Boutique Bloom</p><h1>{shop.name}</h1><p>{shop.description || 'Une boutique en construction, portée par une idée.'}</p></div>{products.length ? <div className="product-grid">{products.map((product) => <Card key={product.id} className="product-card card-lift"><div className="product-cover cover-wine"><Badge tone="demo">{product.category ?? 'Produit'}</Badge><Box /></div><div className="product-info"><h3>{product.title}</h3><p>{product.description}</p><div className="product-meta"><span>Produit numérique</span>{product.promo_price_cents ? <span><span style={{ textDecoration: 'line-through', opacity: 0.6, marginRight: 6 }}>{new Intl.NumberFormat('fr-FR').format(product.price_cents / 100)} {product.currency}</span><strong className="price">{new Intl.NumberFormat('fr-FR').format(product.promo_price_cents / 100)} {product.currency}</strong></span> : <strong className="price">{new Intl.NumberFormat('fr-FR').format(product.price_cents / 100)} {product.currency}</strong>}</div><Link href={`/product/${product.slug}`} className="button button-dark">Découvrir <ArrowLeft size={13} style={{ transform: 'rotate(180deg)' }} /></Link></div></Card>)}</div> : <EmptyState title="Les premiers produits arrivent." description="Cette boutique n’a pas encore publié de ressource." />}</div>;
}